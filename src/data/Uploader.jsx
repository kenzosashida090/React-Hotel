...
async function deleteBookings() {
  const { error } = await supabase.from("bookings").delete().gt("booking_id", 0);
  if (error) console.log(error.message);
}
...
async function createBookings() {
  // Bookings need a guests_id and a cabin_id. We can't tell Supabase IDs for each object, it will calculate them on its own. So it might be different for different people, especially after multiple uploads. Therefore, we need to first get all guests_ids and cabin_ids, and then replace the original IDs in the booking data with the actual ones from the DB
  ...
  const finalBookings = bookings.map((booking) => {
    ...
    return {
      ...
      extras_price,
      total_price,
      guests_id: allGuests_ids.at(booking.guests_id - 1),
